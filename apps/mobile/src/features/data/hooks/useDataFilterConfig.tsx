/**
 * useDataFilterConfig - Hook to generate BaseFilterSheet configuration for data filtering
 * Encapsulates all the domain-specific logic for filtering table documents.
 */
...
export interface UseDataFilterConfigOptions {
  schema?: TableSchema;
  filters: FilterExpression[];
  onChangeFilters: (filters: FilterExpression[]) => void;
}

export interface UseDataFilterConfigResult {
  config: BaseFilterSheetConfig<FilterClause>;
  activeFilters: FilterExpression;
  handleFiltersChange: (newFilters: FilterExpression) => void;
}
...
export function useDataFilterConfig({
  ...
  filters,
  onChangeFilters,
}: UseDataFilterConfigOptions): UseDataFilterConfigResult {
  ...
  const config: BaseFilterSheetConfig<FilterClause> = useMemo(
    () => ({
      ...
      renderSelectMode: (onSelect, editingClauseId, editingPart) => {
        ...
        if (editingPart === "operator" && editingClauseId) {
          const editingClause = activeFilters.clauses?.find(
        ...
        const usedFields = new Set(
          activeFilters.clauses?.map((c) => c.field) ?? [],
        );

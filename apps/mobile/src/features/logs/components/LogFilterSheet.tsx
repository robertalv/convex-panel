/**
 * LogFilterSheet - Bottom sheet for filtering logs
 * Follows the same pattern as DataFilterSheet with pill-based filters
 */
...
export function LogFilterSheet({
  ...
  const getStatusColor = (status: string): string => {
    ...
    if (status === "error" || status === "failure") return theme.colors.error || "#ef4444";
  ...
  const config: BaseFilterSheetConfig<LogFilterClause> = {
    ...
    renderSelectMode: (onSelect, editingClauseId, editingPart) => {
      ...
      if (editingPart === "value" && editingClauseId) {
        const editingClause = filters.clauses.find((c) => c.id === editingClauseId);

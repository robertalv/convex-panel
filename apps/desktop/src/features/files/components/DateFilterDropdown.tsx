...
import { usePortalTarget } from "../../../contexts/PortalContext";
...
export const DateFilterDropdown: React.FC<DateFilterDropdownProps> = ({
  ...
  const [selectedStartDate, setSelectedStartDate] = useState<Date | null>(
    value.type === "custom" && value.startDate
  ...
  const renderCalendar = (month: number, year: number, isFirst: boolean) => {
    ...
    const isDateSelected = (
      ...
      if (selectedEndDate) {
        const normalizedEnd = normalizeDate(selectedEndDate);
        return (
          date.getTime() === normalizedStart.getTime() ||
    ...
    <>
      ...
      {isOpen &&
        ...
        createPortal(
          <div
            ref={dropdownRef}
            className={`cp-theme-${resolvedTheme}`}
            style={{
              position: "fixed",
              top: `${position.top}px`,
            ...
            >
              ...
              >
                {PRESET_OPTIONS.map((option) => {
                  ...
                  const range =
                    option.type !== "any" ? getDateRange(option.type) : null;
              ...
              >
                ...
                {renderCalendar(
                  secondMonthData.month,
                  secondMonthData.year,
                  false,
                )}
              </div>
            </div>

...
export const dashboardFetch: DashboardFetch = async (input, init) => {
  ...
  console.log(`[DashboardAdapter] Fetching ${url} (isTauri: ${isTauri()})`);

  const controller = new AbortController();
  const timeoutId = setTimeout(() => {
    console.error(`[DashboardAdapter] Request to ${url} timed out after 15s`);
    controller.abort();
  }, 15000);
  ...
  try {
    let response;
    if (isTauri()) {
      console.log(`[DashboardAdapter] Using tauriFetch for ${url}`);
      response = await tauriFetch(input, newInit);
    } else {
      console.log(`[DashboardAdapter] Using native fetch for ${url}`);
      response = await fetch(input, newInit);
    }
    console.log(`[DashboardAdapter] Response for ${url}: ${response.status}`);
    return response;
  } catch (error) {
    console.error(`[DashboardAdapter] Error fetching ${url}:`, error);
    throw error;
  } finally {
...
async function discoverAuthEndpoints(fetcher: DashboardFetch) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to discover auth configuration: ${response.status} ${errorText}`,
    );
  }
...
export async function startDeviceAuthorization(
  ...
  if (!deviceResponse.ok) {
    ...
    throw new Error(
      `Failed to start device authorization: ${deviceResponse.status} ${errorText}`,
    );
  }
...
export async function exchangeForDashboardToken(
  ...
  const hostname =
    typeof window !== "undefined" && navigator?.userAgent?.includes("Tauri")
...
export function createDashboardAdapter(
  ...
  return {
    ...
    listProjects: (token: string, teamId: number) =>
      getProjects(token, teamId, fetcher),
    listDeployments: (token: string, projectId: number) =>
      getDeployments(token, projectId, fetcher),
  };
}

...
import { ROUTES, SYSTEM_QUERIES } from "../api-constants";
...
export async function getEnvironmentVariable(
  ...
  try {
    ...
    throw new Error(
      `Failed to get environment variable: ${error?.message || "Unknown error"}`,
    );
  }
}
...
export async function getAllEnvironmentVariables(
  ...
  try {
    ...
    throw new Error(
      `Failed to get environment variables: ${error?.message || "Unknown error"}`,
    );
  }
}
...
export async function updateEnvironmentVariablesViaHTTP(
  ...
  const response = await fetch(
    `${deploymentUrl}${ROUTES.UPDATE_ENVIRONMENT_VARIABLES}`,
    {
      method: "POST",
      headers: {

...
export async function fetchProfile(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to fetch profile: ${response.status}, message: ${error.message || "Unknown error"}`,
    );
  }
...
export async function getTokenDetails(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to get token details: ${response.status}, message: ${errorMessage}`,
    );
  }
...
export async function listTeams(
  ...
  if (!response.ok) {
    const error = await response
      ...
      .catch(() => ({ message: "Unknown error" }));
    throw new Error(
      `Failed to list teams: ${response.status}, message: ${error.message || "Unknown error"}`,
    );
  }
...
export async function getTeamFromDeployment(
  ...
  if (!response.ok) {
    const error = await response
      ...
      .catch(() => ({ message: "Unknown error" }));
    throw new Error(
      `Failed to get team from deployment: ${response.status}, message: ${error.message || "Unknown error"}`,
    );
  }
...
export async function fetchTeams(
  ...
  if (!response.ok) {
    const error = await response
      ...
      .catch(() => ({ message: "Unknown error" }));
    throw new Error(
      `Failed to fetch teams: ${response.status}, message: ${error.message || "Unknown error"}`,
    );
  }
...
export async function fetchProjects(
  ...
  if (!response.ok) {
    const error = await response
      ...
      .catch(() => ({ message: "Unknown error" }));
    throw new Error(
      `Failed to fetch projects: ${response.status}, message: ${error.message || "Unknown error"}`,
    );
  }
...
export async function fetchDeployments(
  ...
  if (!response.ok) {
    const error = await response
      ...
      .catch(() => ({ message: "Unknown error" }));
    throw new Error(
      `Failed to fetch deployments: ${response.status}, message: ${error.message || "Unknown error"}`,
    );
  }
...
export async function fetchProjectInfo(
  ...
  if (deploymentName && authToken) {
    try {
      ...
      const provisionHost =
        typeof process !== "undefined" && process.env?.CONVEX_PROVISION_HOST
    ...
    try {
      ...
      try {
        await callConvexQuery(
          deploymentUrl,
          authToken,
          "_system/frontend/convexSiteUrl",
          {},
        );
      } catch (err) {
        // Ignore errors
      ...
      if (adminClient) {
        try {
          const projectInfo = await adminClient.query(
            ...
            return {
              ...
              project: projectInfo.project
                ? {
                    ...
                    teamId:
                      projectInfo.project.teamId?.toString() ||
                      projectInfo.team?.id?.toString() ||
                      "",
                  }
                : undefined,
            };

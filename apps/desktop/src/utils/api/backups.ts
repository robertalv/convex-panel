...
import {
  ...
  SYSTEM_QUERIES,
  CONVEX_API_DOMAIN,
  CONVEX_PROVISION_DOMAIN,
} from "../api-constants";
import type { CloudBackupResponse, PeriodicBackupConfig } from "./types";
...
export async function createBackup(
  ...
  const path = ROUTES.REQUEST_CLOUD_BACKUP.replace(
    "{deploymentId}",
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to create backup: ${response.status}, message: ${errorMessage}`,
    );
  }
...
export async function listBackups(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to list backups: ${response.status}, message: ${errorMessage}`,
    );
  }
...
export async function getBackup(
  ...
  const baseUrl = `https://${CONVEX_PROVISION_DOMAIN}/`;
  const path = ROUTES.GET_CLOUD_BACKUP.replace(
    "{backupId}",
    backupId.toString(),
  );
  const endpoint = `${baseUrl}${path}`;
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to get backup: ${response.status}, message: ${errorMessage}`,
    );
  }
...
export async function deleteBackup(
  ...
  const baseUrl = `https://${CONVEX_PROVISION_DOMAIN}/`;
  const path = ROUTES.DELETE_CLOUD_BACKUP.replace(
    "{backupId}",
    backupId.toString(),
  );
  const endpoint = `${baseUrl}${path}`;
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to delete backup: ${response.status}, message: ${errorMessage}`,
    );
  }
...
export async function restoreBackup(
  ...
  const path = ROUTES.RESTORE_FROM_CLOUD_BACKUP.replace(
    "{deploymentId}",
    deploymentId.toString(),
  );
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to restore backup: ${response.status}, message: ${errorMessage}`,
    );
  }
 ...
 */
export async function listSnapshotImports(adminClient: any): Promise<any[]> {
  const result = await adminClient.query(
    SYSTEM_QUERIES.LIST_SNAPSHOT_IMPORTS,
    {},
  );
 ...
 */
export async function getLatestRestore(adminClient: any): Promise<any | null> {
  const imports = await listSnapshotImports(adminClient);
  const cloudRestores = imports.filter(
    (imp: any) => imp.requestor?.type === "cloudRestore",
  );
...
export async function confirmSnapshotImport(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to perform import: ${response.status}, message: ${errorMessage}`,
    );
  }
}
...
export function getBackupDownloadUrl(
  ...
  const path = ROUTES.GET_BACKUP_DOWNLOAD_URL.replace(
    "{snapshotId}",
    snapshotId,
...
export function downloadBackup(
  ...
  if (backup.state !== "complete") {
    throw new Error(
      "Backup is not ready for download. Current state: " + backup.state,
    );
  }
...
export async function configurePeriodicBackup(
  ...
  const path = ROUTES.CONFIGURE_PERIODIC_BACKUP.replace(
    "{deploymentId}",
    deploymentId.toString(),
  );
  const endpoint = `https://${CONVEX_API_DOMAIN}${path}`;
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to configure periodic backup: ${response.status}, message: ${error.message || "Unknown error"}`,
    );
  }
}
...
export async function getPeriodicBackupConfig(
  ...
  try {
    const path = ROUTES.GET_PERIODIC_BACKUP_CONFIG.replace(
      "{deploymentId}",
      deploymentId.toString(),
    );
    const endpoint = `https://${CONVEX_API_DOMAIN}${path}`;
    ...
    if (!response.ok) {
      ...
      const errorText = await response.text();
      throw new Error(
        `Failed to get periodic backup config: ${response.status} - ${errorText}`,
      );
    }
...
export async function disablePeriodicBackup(
  ...
  const path = ROUTES.DISABLE_PERIODIC_BACKUP.replace(
    "{deploymentId}",
    deploymentId.toString(),
  );
  const endpoint = `https://${CONVEX_API_DOMAIN}${path}`;
  ...
  if (!response.ok) {
    const error = await response
      ...
      .catch(() => ({ message: "Unknown error" }));
    throw new Error(
      `Failed to disable periodic backup: ${response.status}, message: ${error.message || "Unknown error"}`,
    );
  }
}

...
import { CONVEX_CLOUD_DOMAIN } from "../api-constants";
...
export function extractProjectName(
  _deploymentUrl: string | undefined,
): string | undefined {
 ...
 */
export function parseAccessToken(authToken: string | undefined): {
  teamSlug?: string;
  projectSlug?: string;
} {
  if (!authToken) return {};
...
export function getTeamTokenFromEnv(): string | null {
  ...
  try {
    ...
    const buildTimeToken =
      typeof __CONVEX_ACCESS_TOKEN__ !== "undefined"
        ? __CONVEX_ACCESS_TOKEN__
        : null;
    if (
      buildTimeToken &&
      typeof buildTimeToken === "string" &&
      buildTimeToken.trim()
    ) {
  ...
  if (!isNext) {
    try {
      ...
      if (
        ...
        // @ts-ignore
        const token =
          import.meta.env.VITE_CONVEX_ACCESS_TOKEN ||
          import.meta.env.CONVEX_ACCESS_TOKEN;

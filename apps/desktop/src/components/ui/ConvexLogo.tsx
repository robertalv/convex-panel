interface ConvexLogoProps {
  ...
  className?: string;
  /**
   * If true, renders a monochrome version suitable for system tray/menu bar
 ...
 * Colors: Yellow (#F5B01A), Purple (#8D2576), Red (#EE342F)
 *
 * With monochrome=true, renders a single-color version using currentColor
 */
...
interface ConvexTrayLogoProps {
  ...
  className?: string;
  /**
   * Color for the logo. Defaults to "currentColor" to adapt to parent's text color.
   * Useful for system tray/menu bar icons.
   */
  color?: string;
}

/**
 * The Convex tray logo (simplified pinwheel design).
 * Designed for system tray/menu bar use at 44x44 pixels.
 * Uses currentColor by default to adapt to the parent's text color.
 */

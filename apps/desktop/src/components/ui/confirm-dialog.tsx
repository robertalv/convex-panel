...
export const ConfirmDialog: React.FC<ConfirmDialogProps> = ({
  ...
  const portalTarget = providedContainer ?? document.body;
  const isInContainer = Boolean(providedContainer);

  // Prevent body scroll when dialog is open (only if not in container)
  useEffect(() => {
    if (!portalTarget || isInContainer) return;
    if (isOpen) {
  ...
  }, [isOpen, portalTarget, isInContainer]);
    ...
    <>
      ...
      <div
        style={{
          ...
          animation: isInContainer
            ? "popupSlideInContainer 0.3s ease-out"

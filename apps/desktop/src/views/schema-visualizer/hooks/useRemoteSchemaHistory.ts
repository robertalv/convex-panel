...
export function useRemoteSchemaHistory({
  ...
  // Extract stable primitive values to prevent re-renders
  const selectedRepoId = selectedRepo?.id ?? null;
  const selectedRepoFullName = selectedRepo?.full_name ?? null;
  const selectedRepoDefaultBranch = selectedRepo?.default_branch ?? null;
  ...
  // Track if we've already initialized to prevent multiple auto-loads
      ...
      if (!branchToUse && branchData.length > 0) {
        ...
        if (initialBranch) {
          ...
          if (branchExists) {
            ...
            persistBranch(selectedRepoFullName, initialBranch);
            console.log(
              `[useRemoteSchemaHistory] 🌿 Using initial branch: ${initialBranch}`,
            );
          } else {
            // Fall back to default branch
            branchToUse = selectedRepoDefaultBranch || "main";
            setCurrentBranch(branchToUse);
            persistBranch(selectedRepoFullName, branchToUse);
            console.log(
  ...
  const setBranch = useCallback(
    (branch: string) => {
      ...
      persistBranch(selectedRepoFullName, branch);
  ...
  useEffect(() => {
    const newRepoId = selectedRepoId;
    ...
    if (currentRepoIdRef.current !== newRepoId) {
      ...
      setCurrentBranch(loadPersistedBranch(selectedRepoFullName));
  ...
  }, [selectedRepoId, selectedRepoFullName, schemaPath]);
  ...
  useEffect(() => {
    // Skip if already initialized for this repo
    if (
      hasInitializedRef.current &&
      selectedRepoId === currentRepoIdRef.current
    ) {
      console.log(
        "[useRemoteSchemaHistory] ⏭️  Skipping auto-load (already initialized for this repo)",
      );
      return;
    }
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [autoLoad, isConnected, selectedRepoId]);

...
export const EnvironmentVariables: React.FC<EnvironmentVariablesProps> = ({
  ...
  const validateDuplicateNames = (
    vars: Array<{ name: string; value: string; id: string }>,
  ): Record<string, string> => {
    ...
    envVars.forEach((envVar) => {
      nameOccurrences.set(
        envVar.name,
        (nameOccurrences.get(envVar.name) || 0) + 1,
      );
    });
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          {isAddingNew && (
            <button
              ...
              disabled={
                isSaving ||
                newVars.filter((v) => v.name && v.value).length === 0 ||
            ...
            >
              <div
                style={{
                  ...
                  height: "48px",
                  backgroundColor:
                    "color-mix(in srgb, var(--color-brand-base) 10%, transparent)",
                  borderRadius: "12px",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
              ...
              >
                <Key size={24} style={{ color: "var(--color-brand-base)" }} />
              </div>
              <h3
                style={{
                  fontSize: "18px",
                  ...
                  >
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          copyToClipboard(envVar.value, `${envVar.name}-value`)
                        }
                        style={{
                          cursor: "pointer",
                          display: "flex",
                      ...
                      >
                        <input
                            ...
                            if (parsedVars.length > 0) {
                              ...
                              if (newVarsWithIds.length > 0) {
                                setNewVars((prev) => {
                                  const combined = [...prev, ...newVarsWithIds];
                                  const errors =
                                    validateDuplicateNames(combined);
                                  setValidationErrors((prevErrors) => ({
                                    ...prevErrors,
                                    ...errors,
                                  }));
                          ...
                          style={{
                            ...
                            borderRadius: "4px",
                            border:
                              hasError && focusedInputId === newVar.id
                                ? "1px solid var(--color-error-secondary)"
                                : hasError

...
import { CONVEX_API_DOMAIN, ROUTES, SYSTEM_QUERIES } from '../constants';
...
export async function fetchDeploymentMetadata(
  ...
  if (adminClient) {
    try {
      ...
      const deploymentInfo = await adminClient.query(SYSTEM_QUERIES.DEPLOYMENT_INFO, {});
...
export async function createDeployKey(
  ...
  const response = await fetch(`https://${CONVEX_API_DOMAIN}${ROUTES.CREATE_DEPLOY_KEY.replace('{deploymentId}', deploymentName)}`, {
    method: 'POST',
    headers: {
...
async function changeDeploymentState(
  ...
  const response = await fetch(`${deploymentUrl}${ROUTES.CHANGE_DEPLOYMENT_STATE}`, {
    method: 'POST',
    headers: {
      'Authorization': `Convex ${adminKey}`,
      'Content-Type': 'application/json',

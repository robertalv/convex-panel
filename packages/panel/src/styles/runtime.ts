...
.cp-skeleton--shimmer {
  ...
  border-radius: 4px;
  animation: cp-skeleton-loading 1.5s infinite ease-in-out;
}
@keyframes cp-skeleton-loading {
  0% {
...
.cp-theme-dark {
  --color-panel-editorLineNumber-foreground: #858585;
...
.cp-theme-light {
  --color-panel-editorLineNumber-foreground: #237893;
...
.cp-bottom-sheet [data-sonner-toast][data-type="info"] {
  ...
  .cp-header-section--left {
    flex: 1 1 auto;
    min-width: 0;
    justify-content: flex-start;
  }
  .cp-header-section--right {
    flex: 0 1 auto;
  ...
  .cp-header-controls .cp-separator {
    height: 20px;
  }
  ...
  @media (max-width: 1024px) {
    ...
    .cp-header-section--right > *:not(.cp-header-controls) {
      flex: 1 1 calc(50% - 8px);
      min-width: 140px;
    }
    .cp-header-section .cp-flex {
      flex-direction: row;
    }
    .cp-header-section--right .cp-separator {
      display: none;
    }
    .cp-header-controls {
      width: 100%;
...
.ace_gutter {
  background-color: var(--color-panel-bg-secondary) !important;
}

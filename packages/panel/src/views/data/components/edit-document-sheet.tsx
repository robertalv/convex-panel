...
import { X, ArrowLeft, ArrowRight, Search, Copy } from 'lucide-react';
import type { TableDocument } from '../../../types';
import { formatValue, formatTimestamp, buildColumnMeta } from './table/data-table-utils';
...
export const EditDocumentSheet: React.FC<EditDocumentSheetProps> = ({
  ...
  const [editingField, setEditingField] = useState<{ fieldName: string; value: any } | null>(null);
  const [editingValue, setEditingValue] = useState<string>('');
  const [isSaving, setIsSaving] = useState(false);
  const [editError, setEditError] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState('');
  const [cachedDocuments, setCachedDocuments] = useState<Map<string, TableDocument>>(new Map());
  ...
  const fetchDocument = useCallback(async (documentId: string) => {
    ...
    if (existingDoc) {
      ...
      setCachedDocuments(prev => new Map(prev).set(documentId, existingDoc));
      return;
    }
    ...
    try {
      ...
      if (fetchedDoc) {
        ...
        setCachedDocuments(prev => new Map(prev).set(documentId, fetchedDoc));
      }
    } catch (error: any) {
  ...
  useEffect(() => {
    ...
    const index = Math.min(currentDocumentIndex, selectedDocumentIds.length - 1);
  ...
  const handleSaveEdit = useCallback(async () => {
    if (!editingField || !currentDocument || !adminClient || !selectedTable || isSaving) {
      return;
    }
    ...
    try {
      ...
      // Update local cache
      const updatedDoc = { ...currentDocument, [editingField.fieldName]: parsedValue };
      setCurrentDocument(updatedDoc);
      setCachedDocuments(prev => new Map(prev).set(currentDocument._id, updatedDoc));
      ...
      const errorMessage = error?.data || error?.message || 'Failed to update field';
      setEditError(errorMessage);
      toast('error', errorMessage);
    } finally {
      setIsSaving(false);
    }
  }, [editingField, editingValue, currentDocument, adminClient, selectedTable, isSaving, onDocumentUpdate]);
  ...
  useEffect(() => {
    ...
    const handleArrowKeys = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowLeft' && currentDocumentIndex > 0) {
        ...
        e.stopPropagation();
        handlePrevious();
      } else if (e.key === 'ArrowRight' && currentDocumentIndex < selectedDocumentIds.length - 1) {
        e.preventDefault();
        e.stopPropagation();
        handleNext();
  ...
  }, [isOpen, editingField, currentDocumentIndex, selectedDocumentIds.length, handleCancelEdit, handlePrevious, handleNext]);
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1 }}>
              ...
              <div style={{
                ...
                {showNavigation ? (
                  <>
                    <span>{selectedDocumentIds.length} {selectedDocumentIds.length === 1 ? 'document' : 'documents'}</span>
                    <span style={{
                      padding: '0 4px',
                      borderRadius: '6px',
            ...
            {isLoadingDocument ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {filteredFields.map((fieldName) => {
                  ...
                  const isSystemField = fieldName === '_id' || fieldName === '_creationTime';
                  const meta = columnMeta[fieldName] || { typeLabel: typeof value, optional: false };
                  
                  // Format display value
                  const isCreationTime = fieldName === '_creationTime' && typeof value === 'number';
                  const displayValue = isCreationTime ? formatTimestamp(value) : formatValue(value);

...
import { Settings, TestTube, CheckCircle2, XCircle, AlertCircle, Loader2, Eye, EyeOff, ChevronDown, ChevronUp, Copy, Check } from 'lucide-react';
import { getAIConfig, setAIConfig, testAIConnection, type AIConfig } from '../../../utils/api/aiAnalysis';
...
export const AIAnalysisSettings: React.FC<AIAnalysisSettingsProps> = ({
  ...
  const handleTest = async () => {
    ...
    try {
      const result = await testAIConnection(adminClient, provider, apiKey.trim(), model.trim());
  ...
  const getEmbeddingModelSuggestions = (): DropdownOption<string>[] => {
    if (provider === 'openai') {
      ...
      return [
        { value: 'text-embedding-3-large', label: 'text-embedding-3-large (3072 dim, recommended)' },
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <p style={{ margin: 0 }}>
              Configure AI-powered log and error analysis. Enable automatic analysis or use on-demand analysis.
            </p>
          </div>
          ...
          {testResult && (
            <div
              style={{
                ...
                alignItems: 'center',
                gap: '8px',
                color: testResult.success ? 'var(--color-panel-success)' : 'var(--color-panel-error)',
                fontSize: '13px',
              }}
            >
          ...
          {config && config.provider !== 'none' && (
            ...
            >
              ...
              >
                ...
                <span style={{ fontWeight: 500 }}>Show current configuration</span>
              </button>
              ...
              {isCredentialsExpanded && (
                ...
                >
                  ...
                  >
                    ...
                    >
                      {config.provider === 'openai' ? 'OpenAI' : config.provider === 'anthropic' ? 'Anthropic (Claude)' : 'None'}
                    </div>
                  </div>
                  ...
                  {config.provider === 'openai' && (
                    ...
                    >
                      ...
                      >
                        {config.embeddingModel || 'text-embedding-3-large (default)'}
                      </div>
                    </div>
                  )}
                  ...
                  >
                    ...
                    >
                      Your API key is stored securely. You can view it here or copy it to clipboard.
                    </p>
                    <div
                      style={{
                        display: 'flex',
                      ...
                      <button
                        onClick={() => {
                          ...
                          if (apiKey.trim()) {
                            ...
                            setSuccessMessage('API key is stored securely and cannot be retrieved. Enter a new key to update it.');
                            setTimeout(() => setSuccessMessage(null), 3000);
                          }
                        }}
                        style={{
                          ...
                          alignItems: 'center',
                          justifyContent: 'center',
                          color: copiedValue === 'api-key' ? 'var(--color-panel-accent)' : 'var(--color-panel-text-muted)',
                        ...
                        title="Copy API key"
                      >
                        {copiedValue === 'api-key' ? <Check size={16} /> : <Copy size={16} />}
                      </button>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div>AI Analysis: {config.enabled ? 'Enabled' : 'Disabled'}</div>
                      <div>Automatic Analysis: {config.automaticAnalysis ? 'Enabled' : 'Disabled'}</div>
                    </div>
                  </div>
                </div>
              )}
          ...
          {provider !== 'none' && (
            <>
              ...
              >
                ...
                >
                  {config ? 'Enter a new API key to update, or leave blank to keep the current key.' : 'Enter your API key for the selected provider.'}
                </p>
                <div
                  style={{
                    width: '100%',
              ...
              >
                ...
                >
                  Recommended: {provider === 'openai' ? 'gpt-4o or gpt-4-turbo (supports JSON mode)' : 'claude-3-opus'} for best results
                </p>
                <div
                  style={{
                    width: '100%',
              ...
              {provider === 'openai' && (
                ...
                >
                  ...
                  >
                    Optional: Used for semantic search in chat. If not set, defaults to text-embedding-3-large.
                  </p>
                  <div
                    style={{
                      width: '100%',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <label
                    ...
                    style={{
                      fontSize: '14px',
                      fontWeight: 600,
                      color: enabled ? 'var(--color-panel-text)' : 'var(--color-panel-text-muted)',
              ...
              <div>
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: 'var(--color-panel-bg-secondary)',
                    color: 'var(--color-panel-text)',
                    cursor: isTesting || !apiKey.trim() || !model.trim() ? 'not-allowed' : 'pointer',
                    opacity: isTesting || !apiKey.trim() || !model.trim() ? 0.5 : 1,
                    display: 'flex',
                    alignItems: 'center',
                    gap: '8px',
                    transition: 'all 0.15s ease',
          ...
          <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
            <button
              type="button"
              onClick={handleSave}
              disabled={isSaving || (provider !== 'none' && (!model.trim() || (!apiKey.trim() && !config)))}
              style={{
                padding: '8px 16px',
                fontSize: '13px',
                fontWeight: 500,
                ...
                backgroundColor: 'var(--color-panel-accent)',
                color: 'var(--color-panel-bg)',
                cursor: isSaving || (provider !== 'none' && (!model.trim() || (!apiKey.trim() && !config))) ? 'not-allowed' : 'pointer',
                opacity: isSaving || (provider !== 'none' && (!model.trim() || (!apiKey.trim() && !config))) ? 0.5 : 1,
                display: 'flex',
                alignItems: 'center',
                gap: '8px',
                transition: 'all 0.15s ease',
              ...
              onMouseEnter={(e) => {
                if (!isSaving && !(provider !== 'none' && (!model.trim() || (!apiKey.trim() && !config)))) {

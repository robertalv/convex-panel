...
import { exposeApi } from "convex-panel/component";
...
export const listComments = query({
  ...
  handler: async (ctx, args) => {
    return await ctx.runQuery(components.convexPanelComponent.lib.list, {
      targetId: args.targetId,
    });
  },
});
...
export const translateComment = action({
  ...
  handler: async (ctx, args) => {
    return await ctx.runAction(components.convexPanelComponent.lib.translate, {
      baseUrl: BASE_URL,
...
// the component's API:
export const { list, add, translate } = exposeApi(components.convexPanelComponent, {
